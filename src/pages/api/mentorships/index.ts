...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createMentorship() {
    ...
    const body = { ...req.body };
    if (body?.mentorship_purchase?.length > 0) {
      const create_mentorship_purchase = body.mentorship_purchase;
      body.mentorship_purchase = {
        create: create_mentorship_purchase,
      };
    } else {
      delete body.mentorship_purchase;
    }
