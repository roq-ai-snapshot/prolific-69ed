...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'content-manager', 'admin'];
  ...
  const { data, error, isLoading } = useSWR<TeamInterface[]>(
    ...
    () => getTeams({ filter: { tenant_id: session?.user?.tenantId } }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Team',
      path: isTenantUser ? `/teams/${data?.[0]?.id}` : '/teams',
      entity: 'team',
    ...
    {
      name: 'Mentorship Purchase',
      path: '/mentorship-purchases',
